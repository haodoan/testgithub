...
#define  TIME_SEND_GPRS     20000
char IP_ADDRESS_SERVER[] =  "183.81.68.253";//"113.22.99.141";//
char PORT_SERVER[] = "3389";
int number_send = 0 ;
void main(void)
{
        ...
        if(event_send_gprs)
        {
            ...
            if(start_GPS()) // time out GPS about 90 second if GPS not fix yet
            {
               ...
               // sendATcommand("AT+CGPSPWR=0", "OK", 2000);
                ...
                sprintf(buff_send_gprs,"????,123456789,1,%s,%s,%s,%s,%s,%d,$$$$",date,latitude,longitude,info_bat.bat_percent,info_bat.vol,number_send++);
    ...
    //          // send mcc mnc cell id lac to server
                memset(buff_send_gprs , '\0',120);
                sprintf(buff_send_gprs,"????,123456789,0,%s,%d,%d,%s,%s,%s,%s,%d,$$$$"\
